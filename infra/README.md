...
| `API_FOOTBALL_API_KEY` (ADR-0094 item 3) | API-Football's API key, from api-football.com's free tier (`SETUP.md` §4). Fed to `deploy.yml`'s `apiFootballApiKey` Bicep parameter, which threads through as the Container App's `ApiFootball__ApiKey` env var (`backend-container-app.bicep`) — `DataSync.ApiFootball.ApiFootballClient`'s only credential, xG Predict's (COMP-15) required precondition for round generation (REQ-1301) and grading (REQ-1305). Shared across environments (one API-Football account), not `DEV_`/`PROD_`-prefixed like the Supabase secrets below. Optional/defaults to empty, same posture as `INCIDENT_REPORT_PAT` above — until it's set, every `IApiFootballClient` call (and so every xg-predict round generation) fails closed per-call rather than the app failing to start or deploy |
