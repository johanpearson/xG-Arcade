...
| `FOOTBALL_DATA_API_KEY` (ADR-0099, replacing ADR-0094's `API_FOOTBALL_API_KEY` — API-Football's free tier turned out to exclude the current season entirely, see that ADR) | football-data.org's API token, from football-data.org's free tier (`SETUP.md` §4). Fed to `deploy.yml`'s `footballDataApiKey` Bicep parameter, which threads through as the Container App's `FootballData__ApiKey` env var (`backend-container-app.bicep`) — `DataSync.FootballData.FootballDataClient`'s only credential, xG Predict's (COMP-15) required precondition for round generation (REQ-1301) and grading (REQ-1305). Shared across environments (one football-data.org account), not `DEV_`/`PROD_`-prefixed like the Supabase secrets below. Optional/defaults to empty, same posture as `INCIDENT_REPORT_PAT` above — until it's set, every `IFootballDataClient` call (and so every xg-predict round generation) fails closed per-call rather than the app failing to start or deploy. If an `API_FOOTBALL_API_KEY` secret still exists from before this swap, it is no longer read by anything and can be deleted |
...
| `PROD_BACKEND_HOSTNAME` | Used by `generate-grid-round.yml`/`generate-path-round.yml` to call scheduled internal endpoints on production, and to call `/internal/sync-players` by hand (its `sync-players.yml` wrapper was deleted, S-130 — see `INTERNAL_JOB_TOKEN` row above) |
...
| `DEV_BACKEND_HOSTNAME` | Used by `generate-grid-round.yml`/`generate-path-round.yml` to call scheduled internal endpoints; also by Tier 1's `ci.yml` for the test-data reset call and E2E test target |
...
| `DEV_TURNSTILE_SITE_KEY` (REQ-717, ADR-0037) | The dev Cloudflare Turnstile site's site key (dash.cloudflare.com → Turnstile) — public/non-secret by design (same as `DEV_SUPABASE_ANON_KEY` above), fed to `deploy.yml`'s `deploy-frontend` job as `VITE_TURNSTILE_SITE_KEY`, which Vite bakes into the built frontend bundle exactly the same way it already does `VITE_API_BASE_URL`. **Not wired through Bicep** — like `VITE_API_BASE_URL`, this is a frontend build-time value passed directly to the `Azure/static-web-apps-deploy` GitHub Action's Oryx build step, not a Container App setting, so it never appears in `main.bicep`/`static-web-app.bicep` at all. See `SETUP.md` step 6 for where the site key comes from and why the *secret* key never belongs in this table (or anywhere in this application's own config) at all — it's pasted directly into Supabase's Auth dashboard instead |
