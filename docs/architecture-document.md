...
| COMP-16 | Core.Social | **Decided (ADR-0103, 2026-09-02).** **Data model scaffolding built (S-208, 2026-09-02):** `FriendRequest`/`Friendship`/`Challenge`/`MatchmakingOptIn` entities, `IFriendRepository`/`IChallengeRepository`/`IMatchmakingOptInRepository` + implementations, and a migration exist in `XGArcade.Data`, registered in `ServiceRegistration.cs` — schema/repository CRUD only, no send/accept/decline or matchmaking-pairing service logic yet (that's S-209/S-210), and no `XGArcade.Core` (`Social/`) service layer exists yet either. An arcade-level component, alongside `Core.Users`/`Core.Leagues`, not behind `IGameModule` — genuinely separate from COMP-17, not folded into it, since friends are conceptually reusable by any future game, not xG-Connect-specific. Owns friend request send/accept/decline (`Friendship`/`FriendRequest`, REQ-1401), direct challenges between friends (`Challenge`, REQ-1402), and random-matchmaking pairing within a 12-hour window (`MatchmakingOptIn`, REQ-1403). A resolved challenge/pairing creates a `ConnectMatch` (COMP-17) directly — never via `Core.Rounds`/`RoundGenerationService` (ADR-0103's second decision: xG Connect's pairwise, on-demand match doesn't fit the `Round`/`League` model). REQ-1411's cross-cutting notification indicator reads from both COMP-16 and COMP-17 through their normal read paths rather than being owned by either. | `XGArcade.Core` (`Social/`), entities/repositories in `XGArcade.Data` |
| COMP-17 | Games.XGConnect | **Decided (ADR-0103, 2026-09-02).** **Data model scaffolding built (S-208, 2026-09-02):** `ConnectMatch`/`ConnectTargetPick`/`ConnectChainStep`/`ConnectChatMessage` entities, `IConnectMatchRepository`/`IConnectChatMessageRepository` + implementations, and a migration exist in `XGArcade.Data`, registered in `ServiceRegistration.cs` — schema/repository CRUD only. No `XGArcade.Games.XGConnect` project, no `IGameModule` implementation, and no target-pick/chain-validation/bust/scoring/resolution/chat service logic exist yet (that's S-211 onward). A fourth game module behind `IGameModule` (ADR-0003), owning target-pick selection (including rejecting an already-directly-connected pair), incremental connection-chain submission with live per-step validation against real career data, the two-strikes-per-step penalty/bust rule, scoring, win/draw/forfeit match resolution, and in-match chat (REQ-1404-1410, `requirements-document.md` §4.15). Persists its own `ConnectMatch` entity, scoped to exactly two participating `UserId`s with a native win/draw/forfeit outcome (REQ-1409) — never a `Round`/`GameKey`+`GameInstanceId` pair, and never scored via `Core.Scoring`'s `FinalPoints`/`IScoringStrategy` (ADR-0103). Implements only the `IGameModule` slice that applies to a non-`Round`-based game — `PurgeUserDataAsync` (REQ-710, ADR-0101's per-module purge hook) — not the round-generation methods (`GenerateInstanceAsync`/`GetCellIdsAsync`/`GetMaxAttemptsForCellAsync`), which are meaningless here and throw/no-op per the same precedent COMP-11/COMP-15 already established for their own inapplicable methods; the exact shape is left to whichever later story first scaffolds the `XGArcade.Games.XGConnect` project and its `IGameModule` implementation (S-211 onward). Whether `ConnectMatch` results ever feed a `Core.Leagues` leaderboard remains an open product decision, explicitly out of ADR-0103's scope. | `XGArcade.Games.XGConnect` (not yet scaffolded as a project), entities/repositories in `XGArcade.Data` |
