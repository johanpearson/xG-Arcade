...
Grading Job → DataSync.Clients (COMP-07, ADR-0094): poll API-Football for
  ...
  → Core.Leagues (COMP-02): leaderboard for GameKey="xg-predict" sorts
    ...
    direction per GameKey. NOT built: GetRankedMembersAsync's separate
    median-based OrderBy (REQ-409/410, the all-time Global League ranking)
    was not part of that migration and remains unconditionally ascending —
    a real, currently-latent gap against REQ-1304's own acceptance text;
    see ADR-0095's amendment and REQ-1304's status note
```
