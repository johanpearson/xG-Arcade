...
| COMP-16 | Core.Social (proposed) | **Proposed — no code exists yet.** Would own friends and challenge/matchmaking: friend request send/accept/decline, direct challenges between friends, and random-matchmaking pairing within a 12-hour window (REQ-1401-1403, `requirements-document.md` §4.15). Not yet decided whether this is a genuinely separate arcade-level component — friends are conceptually available to any future game, not xG-Connect-specific — or folded into COMP-17; that split, and the more fundamental question of whether a pairwise, on-demand match fits the existing `Round`/`League` model (COMP-02/COMP-03) at all or needs a new first-class concept, is flagged in §4.15's own component-boundary note and needs a deliberate ADR before implementation begins. | Not yet assigned |
| COMP-17 | Games.XGConnect (proposed) | **Proposed — no code exists yet.** Would be a fourth game module behind `IGameModule` (ADR-0003): target-pick selection (including rejecting an already-directly-connected pair), incremental connection-chain submission with live per-step validation against real career data, the two-strikes-per-step penalty/bust rule, scoring, win/draw/forfeit match resolution, and in-match chat (REQ-1404-1410, `requirements-document.md` §4.15). No `IGameModule` implementation, entities, or endpoints exist yet. | Not yet assigned |
