...
own daily cron (`generate-grid-round.yml`/`generate-path-round.yml`, split
from a single `generate-round.yml` as of S-136/ADR-0072) is the flow's only
production trigger: it calls `POST /internal/generate-round`
...
All three legs now describe real, tested behavior, minus the HTTP-facing
submission-endpoint wiring and the round-generation/leaderboard wiring
noted inline below — REQ-1306 (confirm-and-lock) remains entirely
unbuilt, matching §4.14's own intro status)
