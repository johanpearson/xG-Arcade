...
| COMP-18 | Games.XGHigherLower | **Decided (ADR-0110, 2026-09-09).** **`XGArcade.Games.XGHigherLower` project scaffolded (2026-09-09):** `XGHigherLowerGameModule` is now a real, registered `IGameModule` implementation (`GameKey = "xg-higher-lower"`, `IGameModuleResolver.Resolve("xg-higher-lower")` returns it) — a fifth game, alongside Games.XGGrid (COMP-05), Games.XGPath (COMP-11), Games.XGPredict (COMP-15), and Games.XGConnect (COMP-17). Unlike COMP-17, this game DOES fit the existing `Round` model (ADR-0110, not a new first-class concept like `ConnectMatch`) — `GenerateInstanceAsync` (REQ-1501/1502/1503), `ScoreSubmissionAsync` (REQ-1504), and `GetCellIdsAsync` (ADR-0021's round-close unanswered-cell handling) all throw `NotImplementedException` rather than `NotSupportedException`, since these ARE the methods this module will eventually implement once REQ-1501-1505's category-eligibility/comparator-sequence-generation/streak-progression logic is built — not a "permanently inapplicable" case the way COMP-17's own round-generation methods are. `GetMaxAttemptsForCellAsync` also throws `NotImplementedException`, flagged as an open question the same way `XGPredictGameModule`'s own implementation already is (REQ-1504 caps a whole *attempt* at the Round's configured comparator count, not a per-cell retry count, so whether this method even applies to this game's shape is undecided). `GetCellCategoryTypesAsync` throws `NotSupportedException` (REQ-215's row/col category concept genuinely does not apply — a comparison is one numeric stat value against one fixed active category, not two independent category axes, mirroring COMP-11/COMP-15's own precedent), and `ResolveWrongGuessPlayerAsync` returns `null` unconditionally (REQ-216 does not apply — this game has no player-name-guessing or autocomplete surface at all, per `requirements-document.md` §4.16's own "Why this game" note). `PurgeUserDataAsync` is a real no-op, not a throw — no per-user data model exists for this game yet (no persisted instance/attempt entity has been built), and this module IS registered as a real `IGameModule`, so `AccountDeletionService` (COMP-01) calls it for every deleted user; throwing here would break account deletion platform-wide, not just flag a gap (mirroring `GridGameModule`'s/`XGPathGameModule`'s own no-op precedent for a game with no per-user table of its own). References `XGArcade.Data` (boundary rule 1) in anticipation of REQ-1501's `PlayerAttribute`/`PlayerOverride` (COMP-06) reads — no new external data source and no `PlayerNameIndex`/COMP-10 read, since this game has no autocomplete concept. No xG-Higher/Lower-specific persisted entity, repository, or data-model shape exists yet; no `RoundSchedulingOptions`, `IScoringStrategy`, or `GuessSubmissionAllowedGameKeys` registration exists yet either — deliberately, since nothing should call `GenerateInstanceAsync`/`ScoreSubmissionAsync` in production before REQ-1501-1505's real implementation lands. See `docs/requirements-document.md` §4.16 (REQ-1501 through REQ-1505) for the full Given/When/Then behavior this component must eventually implement. | `XGArcade.Games.XGHigherLower` |
