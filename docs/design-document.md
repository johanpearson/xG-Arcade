...
- **Tab bar**: same plain underline-tab pattern (`role="tablist"`,
  ...
  would just restate the same information twice on screen at once). **A
  fourth tab, "Matches," was added by S-218 — see SCREEN-16 for its own
  content**, kept plain-label for the same "no restated count" reasoning as
  the three above.
...
│ Player B2C3D4E5 — Active — Your move │
│                          [View match] │
└───────────────────────────────┘
        │ (View match)
        ▼
...
- **Matches tab (`MatchesTab.tsx`)** — every match from `GET /matches`,
  ...
  since this is simply a fourth tab on it). Each row: the opponent's
  `shortUserIdOrDeleted()` label (a new nullable-safe wrapper around
  SCREEN-15's own `shortUserId()`, added to `shortUserId.ts` — this
  endpoint's `opponentUserId` goes null post-REQ-710 anonymization, unlike
  any of SCREEN-15's own response shapes), a plain status label ("Awaiting
  target picks" / "Active" / "Resolved"), the outcome once resolved ("(You
  won)" / "(You lost)" / "(Draw)"), and, when `awaitingMyAction` is true, an
  inline "— Your move" text label in `accent-green-text` — text, not
  color-only, per §6. Empty state (§5, "invitations"): "You don't have any
  xG Connect matches yet. Challenge a friend or opt into matchmaking to
  start one." — pointing at the two tabs that actually create a match.
...
- **In-match chat (`MatchChat.tsx`, REQ-1410).** Rendered unconditionally by
  ...
  `App.tsx`'s `currentUser.id`, `shortUserIdOrDeleted()` otherwise — message
  text, and a `mono-figure` timestamp) plus a `<textarea>` + "Send message"
