# ADR-0025: Player pool restricted to male footballers born in 1939 or later
...
football history that unexpectedly surfaces an unfamiliar early-20th-
century or women's-football name breaks the "I should be able to reason my
way to this answer" experience the game depends on. This is a scoping
decision about what the *product* covers, not a bug in how existing code
already works — Q937857 ("association football player") on its own
doesn't imply "male," and nothing in the existing SPARQL queries
constrained by date at all.

Two decisions had to be made concrete rather than left as vague policy:
how "male" is expressed against Wikidata's actual data model, and where
the era cutoff sits. An initial pass implemented the era cutoff as a
rolling window ("the latest 100 years," recomputed from the current date
on every query) before the user corrected course: a **fixed** date, 1939,
is the actual requirement — not a window that keeps sliding forward every
year.
...
require two additional triple/filter pairs: `?player wdt:P21 wd:Q6581097`
(P21 = sex or gender, Q6581097 = male) and `?player wdt:P569 ?dateOfBirth`
with `FILTER(?dateOfBirth >= "1939-01-01T00:00:00Z"^^xsd:dateTime)` (P569 =
date of birth). The cutoff is a `private const string` on `WikidataClient`
— a fixed date baked into the query text, not computed from the current
time — so there is no `TimeProvider`/clock dependency involved at all.
...
and rebuilt from scratch under the new queries: a `purge-player-pool` CLI
verb (S-038) does the bulk delete, gated behind a required, exact
...
|---|---|---|---|
| Rolling window relative to "now" (e.g. "the latest 100 years," `TimeProvider.GetUtcNow().AddYears(-N)`, recomputed on every query) | Self-maintaining — never needs a manual update as real time passes | Not what was actually asked for once clarified: the requirement is a specific, fixed point (1939), not a window that keeps drifting forward every year — a rolling window would eventually include players born well after 1939 and still call that compliant, which isn't the intent | Not chosen: the user explicitly corrected this after an initial implementation used it — a fixed date is the real requirement, not a moving approximation of one |
| Fixed date, 1939-01-01 (chosen) | Matches the actual requirement exactly; simpler code — no `TimeProvider` dependency, no clock injection needed in tests, a plain `const string` | Will never automatically track "the latest N years" as time passes — if the intent was ever "roughly a century of history," this cutoff becomes an increasingly large window as real time moves forward past 2039 | Accepted: this is a deliberate, fixed product boundary (not explained as "N years back from today"), so a fixed literal is the correct and simplest implementation of it |
| Career/active-period filter instead of date of birth | Closer to "footballers from a given era" in spirit — a player born just before 1939 who was still actively playing recently would be included | Wikidata's career-span properties (P54 qualifiers like start/end time) are far less consistently populated across players than P569 (date of birth) — this would silently and unpredictably exclude real, in-scope players with incomplete career-date data much more often than the date-of-birth proxy does | Not chosen: date of birth is a reliable single-property proxy, and the fixed 1939 cutoff is unambiguous enough that the proxy's imprecision at the margins doesn't materially change what the game covers |
...
- Positive: the player pool going forward is scoped to what the game
  actually needs — no more surfacing an unfamiliar early-20th-century or
  women's-football name a player has no realistic way to reason their way
  to.
- Positive: no clock/`TimeProvider` dependency for this specific filter —
  the fixed-date implementation is simpler than the rolling-window one it
  replaced (no injected-`TimeProvider` test setup needed for the
  date-of-birth assertions).
...
- Negative / trade-offs accepted: the 1939 cutoff is a fixed point, not a
  self-maintaining window — if the actual intent shifts over time (e.g.
  "keep it to roughly a century of history"), someone will need to
  deliberately revisit and bump this literal; nothing in the system will
  ever do that automatically. Revisit this ADR if that assumption changes.
...
- Follow-up: if a future Tier 1 data source (API-Football, ADR-0011's
  ...
  male/born-1939-or-later filtering applied independently — this ADR's
...
(male) triple and a `wdt:P569`/`FILTER(?dateOfBirth >=
"1939-01-01T00:00:00Z"^^xsd:dateTime)` pair — do not add a query path that
bypasses these filters without discussing it first, and do not reintroduce
