...
public class RoundCloseServiceScoringTests
{
    ...
    [Test]
    public async Task REQ205_CloseRoundAsync_LoneCorrectGuessOnCell_LocksFinalUniquenessScoreZeroAndFinalPointsZero()
    {
        // A lone correct guesser is 100% of the correct-guess population
        // (1/1) — per UniquenessCalculator, that's 0% unique, hence 0
        // points. Intended by the formula, not a bug.
        ...
        Assert.That(persisted.FinalUniquenessScore, Is.EqualTo(0.0));
        Assert.That(persisted.FinalPoints, Is.EqualTo(0));
    }

    [Test]
    public async Task REQ205_CloseRoundAsync_TwoCorrectGuessesWithDifferentAnswersOnSameCell_EachLocksFiftyPercentAndFiftyPoints()
    {
        var round = await SeedRoundAsync(DateTime.UtcNow.AddDays(-1), DateTime.UtcNow.AddDays(1));
        var cellId = Guid.NewGuid();
        ...
        Assert.That(persistedFirst.FinalUniquenessScore, Is.EqualTo(0.5));
        Assert.That(persistedFirst.FinalPoints, Is.EqualTo(50));
        Assert.That(persistedSecond.FinalUniquenessScore, Is.EqualTo(0.5));
        Assert.That(persistedSecond.FinalPoints, Is.EqualTo(50));
    }
    ...
    // instead of the correct 0 (1 - 1/1, denominator = correct guesses
    // only). Pinning this down so it can never silently regress.
    [Test]
    public async Task REQ205_CloseRoundAsync_CellWithOneCorrectAndOneIncorrectGuess_LocksCorrectGuessAtZeroNotFiftyPercent()
    {
        var round = await SeedRoundAsync(DateTime.UtcNow.AddDays(-1), DateTime.UtcNow.AddDays(1));
        var cellId = Guid.NewGuid();
        var playerAnswerId = Guid.NewGuid();
