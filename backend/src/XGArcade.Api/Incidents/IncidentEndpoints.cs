...
public static class IncidentEndpoints
{
    ...
    public static void MapIncidentEndpoints(this WebApplication app)
    {
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            if (request.Route is { Length: > RouteMaxLength })
            {
                return Results.Problem(
                    title: "Route is too long",
                    detail: $"route must be at most {RouteMaxLength} characters.",
                    statusCode: StatusCodes.Status400BadRequest);
            }
            ...
            var result = await incidentReportService.SubmitAsync(
                user.Id, request.Description.Trim(), request.Route, cancellationToken);
...
// Route: the current screen/page the client was on when the report was
// filed (e.g. "settings") — optional context only, never trusted for
// anything beyond display in the created issue's body (REQ-903).
public record SubmitIncidentReportRequest(string Description, string? Route);
