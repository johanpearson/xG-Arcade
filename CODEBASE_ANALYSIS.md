...
| 8 | `LeaderboardScreen.tsx` (1,130 lines, 6 `useEffect`, low churn) | P4 — watch | Large but stable; per this report's own doctrine, not an action item until something else touches it. **Superseded 2026-08-16 (S-121)** — split anyway once S-120's hook migration touched the file; see §5 |
...
- **#8 — `LeaderboardScreen.tsx`**: 1,130 lines, 6 `useEffect`, only 1 `useState` (a single state-object pattern, not 16 independent `useState` calls — actually a *healthier* shape than `AdminScreen.tsx`'s pre-S-103 form). Low churn (3 commits). No action. **Superseded (2026-08-16, S-121):** the file was split anyway — not because this watch-only call was wrong on its own churn-based terms, but because S-120's `useAuthedFetch` migration touched the file regardless, making a combined pass cheaper than two separate ones (see `docs/backlog.md` S-121). Now `LeaderboardScreen.tsx` (261 lines, thin orchestrator) plus `AllTimeLeaderboard.tsx`/`LiveLeaderboard.tsx`/`PastRoundsLeaderboard.tsx`/`WindowedLeaderboard.tsx`/`LeaderboardRowsList.tsx`. This line is corrected in place rather than deferred to the next full sweep since leaving a flatly-wrong "No action" note on record would be worse; a proper re-verified revision (fresh churn/line counts across the whole tree) is still `code-health-auditor`'s job, not redone here.
