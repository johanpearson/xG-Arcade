...
- **2026-09-13 (this revision, `code-health-auditor` periodic sweep, requested 9.0 target, weighted toward newer code)** — **Step 0:** verified Epics 22/23/24 (S-165–S-176, all 11 of #1a/#11–#14's fixes plus 7 further findings from two same-day 2026-08-23 follow-up sweeps) are genuinely fully shipped — not by `git log`, since this repository's own history only reaches back to 2026-09-02 (a repository-history quirk; every SHA those stories' "Built as" notes cite predates that boundary), but by reading every claimed file directly: `PlayerCacheWarmingService.cs`'s `SweepPairsAsync`, `PlayerCareerPrefetchService.cs`'s `SweepAsync`, `CliVerbDispatcher.cs`'s `BuildWikidataClient` (now called from 8 handlers, up from 5, every new one reusing it correctly), `frontend/src/lib/apiClient.ts`'s `apiRequest<T>`, `frontend/src/lib/useRoundFetch.ts`, both `GridGameModule.cs`'s/`GridLiveLookupDispatcher.cs`'s unused-`ILogger` removal, `architecture-document.md`'s COMP-07 fix, `infra/bicep/main.parameters.json`'s deletion, and `.github/actions/run-cli-verb`/`trigger-round-generation` — all confirmed present and correct. **None re-flagged, none re-done.** Since 2026-08-23, three entire new games landed (xG Predict, xG Connect, xG Higher/Lower), plus REQ-1420 (dispute-visibility gate), S-234/ADR-0114 (round-generation renewal), avatar uploads, and friends/challenges/matchmaking — this pass weighted its search toward that code, per the requester's explicit direction, rather than re-deriving already-settled ground. **Two new findings** — see #15/#16 below — plus extensive "investigated and cleared" detail for the new games and high-churn registries, recorded so the next sweep doesn't re-derive them. `npm install` (fresh), `npm run test -- --run` (993/993, 74 files, up from 647/647), `tsc -b`, `oxlint` (3 pre-existing, deliberate warnings, not a finding) all ran live and clean. `npm audit --omit=dev`: 0 vulnerabilities (production clean, re-verified). Full `npm audit`: 3 dev-only advisories (up from 1 — `nanoid@<3.3.18` unchanged plus a new `@vitest/mocker` path-traversal advisory), both Dependabot's lane. No `dotnet` SDK, confirmed again — no backend code changed this pass (findings/planning only). New work tracked as `docs/backlog.md` Epic 30 (S-235/S-236).
- **2026-09-14 (this revision, `code-health-auditor` periodic sweep)** — **Step 0:** `git log 992014f..HEAD` (the commit behind the 2026-09-13 revision) shows only 5 non-merge commits since, all already tracked as Epic 30 S-235/S-236/S-237 — verified genuinely shipped by reading current code/config, not re-discovered: `App.tsx` 826→690 lines with `useAppNavigation.ts` (272 lines) extracted (#15 below, now resolved); `docs/backlog.md` has exactly one `## Epic 13` header left, the later occurrence renumbered to `## Epic 31` (#16 below, now resolved); `.github/workflows/ci.yml`'s `code-health-budget` job (S-237, ADR-0115, `jscpd`-based) exists and runs a real scan. S-238 (backend duplicate-shape scanner survey) correctly remains open — blocked on `dotnet` SDK access this sandbox doesn't have, not actionable this pass. **No new duplication/god-file/boundary finding surfaced** — no feature commits landed in the window this sweep covers, only the three remediation stories above. `npm install` (fresh), `npx vitest run` (993/993, 74 files, unchanged from 2026-09-13), `tsc -b`, `oxlint` (3 pre-existing, deliberate warnings, unchanged) all ran live and clean. **New this pass, at the requester's explicit ask:** a REQ-level test-coverage-gap audit across NUnit unit, NUnit+`WebApplicationFactory` API, Vitest, and Playwright — full detail in `CODE_HEALTH_ASSESSMENT.md` §5 rather than duplicated here (this document's own convention is priority-ranked *code* findings; coverage-existence gaps are a different lens, kept in the numeric-score report). Two concrete gaps found (REQ-603 concurrent-guess correctness, REQ-702/703/704/706 confirmation-email flow) plus a broader E2E-scope gap (no Playwright coverage outside the five games' own play flow) — all flagged for `test-writer` directly, not turned into a backlog epic story, since new test coverage falls outside this lineage's pure-refactor/no-new-coverage house rule. No `dotnet` SDK, confirmed again.
...
**Overall health: Good, and stable — 8.5/10 this pass (+0.2 since 2026-09-13).** Epics 17/21/22/23/24/30 are now all settled, verified ground. **Items 1-16 below are all archival** — #15 (`App.tsx`) and #16 (the duplicate Epic 13 header) are now both resolved (S-235/S-236), confirmed by reading current code/config directly rather than trusted from their own notes. **No new code-duplication/god-file/boundary finding surfaced this pass** — no feature commits landed since the 2026-09-13 revision, only the three remediation stories (S-235/S-236/S-237) that revision itself planned. S-238 (the natural backend counterpart to S-237's frontend CI backstop) correctly remains open, blocked on `dotnet` SDK access. **New this pass, at the requester's explicit ask:** a REQ-level test-coverage-gap audit across all four testing levels — full detail in `CODE_HEALTH_ASSESSMENT.md` §5, summarized: REQ-603 (concurrent-guess uniqueness correctness) and REQ-702/703/704/706 (confirmation-email flow) have no dedicated test despite `docs/requirements-document.md` calling for one; E2E (Playwright) coverage exists only for the five games' own play flows, with no spec for auth/signup, leaderboard, admin, friends/challenges, or account deletion. These are flagged for `test-writer` directly, not written up as a backlog epic story here, since writing new tests is new coverage and this lineage's epics are explicitly pure-refactor/no-new-coverage.
...
`PlayerCacheWarmingService.cs` (`backend/src/XGArcade.Games.XGGrid/`, 388 lines, 4 commits) is almost entirely two nested loops in `WarmAsync` that duplicate the exact same 5-branch decision tree end to end: the Country×Club loop (~lines 169-269) and the Club×Club loop (~lines 271-349) each check already-valid, then confirmed-low-from-a-fully-swept-pool (ADR-0078/S-160), then previously-confirmed-low, then persistent-technical-failure, and only then run a live Wikidata lookup and persist the outcome — differing only in which two `AttributeType`/name pairs and which `IWikidataLookupService` method (`LookupAndPersistAsync` vs. `LookupAndPersistClubClubAsync`) are used. This is the *third* occurrence of this specific "country/club-pair sweep" duplication shape in the codebase (after `WikidataClient.cs`'s HTTP handling in 2026-08-10/Epic 7, and #1a/`PlayerCareerPrefetchService.cs` above) — not a coincidence, the same underlying "two categories, sweep every pair" domain shape keeps recurring at different call sites as the codebase grows. Well-tested (775-line `PlayerCacheWarmingServiceTests.cs`). **Fix:** extract a shared `SweepPairsAsync`-shaped helper both loops call, preserving every counter/log line verbatim — `docs/backlog.md` Epic 22 S-166. **✅ Resolved (S-166), confirmed this revision** — `PlayerCacheWarmingService.cs` is now 367 lines with `SweepCountryClubPairsAsync`/`SweepClubClubPairsAsync` both thin wrappers over a shared private generic `SweepPairsAsync<TLeft, TRight>`.

### #12 (P2): `CliVerbDispatcher.cs` — 5 of 14 handlers duplicate their own Wikidata-client bootstrap

`CliVerbDispatcher.cs` (769 lines) is the single highest-churn file in the entire repository (13 commits) — both the 2026-08-18 and 2026-08-22 revisions confirmed its *verb-registry* shape (S-112's `Verbs` dictionary + one handler per verb, S-114's shared `BuildDbContext`/`BuildLoggerFactory`) is healthy and unchanged, and that verdict still holds. This is a different, narrower finding one level down, only visible on a full re-read rather than a `git log --since` diff check: 5 of the 14 handlers (`HandleWarmPlayerCacheAsync`, `HandleImportPlayerNameIndexAsync`, `HandleBackfillPlayerPhotosAsync`, `HandleBackfillPlayerPositionBirthYearAsync`, `HandlePrefetchPlayerCareersAsync`) each construct a bare `HttpClient`, call `WikidataHttpClientConfiguration.Configure` on it, then `new WikidataClient(...)` with a `logger:` from the handler's own `BuildLoggerFactory()` — the one piece of per-handler boilerplate S-114's own extraction didn't reach (it only pulled `BuildDbContext`/`BuildLoggerFactory` themselves out, not what callers do with them). **Fix:** extract a shared `BuildWikidataClient(ILoggerFactory, TimeSpan? queryTimeout)` helper mirroring `BuildDbContext`/`BuildLoggerFactory`'s own shape — `docs/backlog.md` Epic 22 S-167. **✅ Resolved (S-167), confirmed this revision and holding under growth** — `BuildWikidataClient` exists and is now called from **8** handlers (up from the original 5; 3 new ones added since for xG Higher/Lower's international-stats/trophy-stats backfills), every new call site reusing it correctly rather than re-inlining the bootstrap.

### #13 (P3): `frontend/src/lib/*.ts` — 47 call sites duplicate the same fetch+error-check+parse shape

Every domain file under `frontend/src/lib/` that talks to the backend (`admin.ts` 18 call sites, `auth.ts` 9, `announcements.ts` 5, `leaderboard.ts` 5, `rounds.ts` 4, `leagues.ts` 3, `incidents.ts` 2, `path.ts` 1 — 47 total across 8 files) hand-rolls the same shape at every call site: build a headers object, call `fetch(...)`, check `!response.ok` and call the already-shared `throwApiError` if so, then parse the JSON body. `apiClient.ts`'s own header comment frames itself as "the shared fetch foundation every domain file imports from," but today it only centralizes the *error*-handling half — the *request*-building half is still repeated 47 times by hand. Never flagged before this pass because no single file in `lib/` was ever large enough to be the outlier on its own (S-111, Epic 8, already split the original monolithic `api.ts` by domain) — the duplication is only visible aggregated across the whole directory, which this pass's wider net specifically looked for. Deliberately distinct from `useAuthedFetch.ts` (S-107/Epic 17), which is React-hook-scoped and owns loading/error *state* — these are plain async functions callable from anywhere, so the right fix is a small `apiRequest<T>`-shaped function, not a hook. **Fix:** add a shared request-building helper to `apiClient.ts` — `docs/backlog.md` Epic 22 S-168. **✅ Resolved (S-168), confirmed this revision** — `apiClient.ts` (102 lines) now exports `apiRequest<T>`, used by all 8 domain files; each call site's status-code special-casing (404-as-data, etc.) preserved.

### #14 (P3): `GridScreen.tsx`/`PathScreen.tsx` duplicate their own round-fetch/load-state machinery

`GridScreen.tsx` (529 lines) and `PathScreen.tsx` (357 lines), 5 commits each, independently define an identically-shaped `LoadState` union, an identical round-fetch mount effect (fetch, guard on a `cancelled` flag, treat a 401 as `onAuthError()`, otherwise set an error state), an identical fire-and-forget `warmUpAutocomplete` mount effect, and a `handleViewCompletedRoundLeaderboard` that `PathScreen.tsx`'s own comment states outright "mirrors GridScreen.tsx's `handleViewCompletedRoundLeaderboard` exactly." Not previously flagged because Epic 17's frontend pass was `AdminScreen.tsx`/`App.tsx`-scoped, and these two screens' own `useAuthedFetch.ts` evaluation (correctly rejecting the hook because both screens need to mutate fetched state after a guess submission) asked a different question than "do these two screens duplicate each other" — they do, just not via `useAuthedFetch.ts`. **Fix:** extract a shared `useRoundFetch`-shaped hook, generic over the round/path response type, covering the `LoadState`/fetch-effect/warm-up piece at minimum — `docs/backlog.md` Epic 22 S-169. **✅ Resolved (S-169), confirmed this revision** — new `frontend/src/lib/useRoundFetch.ts` (138 lines) exports `useRoundFetch`/`useAutocompleteWarmup`, both screens now call it; each screen's own `handleViewCompletedRoundLeaderboard` wrapper and genuinely-different guess-submission state were correctly left untouched.

### #15 — `App.tsx` — five new screens re-formed the duplicated-shape pattern `useSession` (S-158) already extracted half of — **✅ Resolved (S-235), confirmed 2026-09-14**

`App.tsx` regrew from 529 lines (post-S-158, 2026-08-22) to **826 lines**
(8 commits since) as five new top-level screens landed one at a time
(xG Predict, xG Connect's entry screen, xG Higher/Lower, Friends, Stats) —
each addition individually well-reasoned and consistent with the file's
own established per-screen pattern (its own doc comments cross-reference
the precedent each one copied). The aggregate, though, is the same
"duplicated shape repeated per near-identical case" pattern this report
has now caught ten times: four separate `[state, setState]` pairs each
paired with their own one-line "seed a target, then `navigateTo`" handler
(`handleViewRoundLeaderboard`, `handleOpenOwnStats`/
`handleSelectPlayerStats`, `handleOpenFriendsTab`), plus the
`SCREEN_HASHES`/`HASH_TO_SCREEN`/`screenForHash`/`navigateTo` hash-sync
mechanism itself — all still living directly in `App`'s own function body,
one level up from where `useSession` (S-158) already extracted this same
file's auth-lifecycle half. Not previously flagged because Epic 17's own
`App.tsx` evaluation was scoped to the auth-lifecycle extraction
specifically, not a re-check of the rest of the file after every
subsequent screen addition. **Fix:** extract a `useAppNavigation`-shaped
hook, mirroring `useSession.ts`'s own placement/export shape — `docs/backlog.md`
Epic 30 S-235. **Confirmed landed by reading current code this revision:**
`App.tsx` is now 690 lines; `frontend/src/lib/useAppNavigation.ts` (272
lines) owns the `Screen` union, hash-sync mechanism, `navigateTo`, and a
new `seedAndNavigate` helper the four handlers each wrap; `App.test.tsx`
passes unmodified (993/993 full-suite run, unchanged from 2026-09-13).

### #16 — `docs/backlog.md` has two colliding `## Epic 13` headers — **✅ Resolved (S-236), confirmed 2026-09-14**

Line 6272 ("Autocomplete: threshold verification + cold-start latency",
containing S-142/S-151) and line 9995 ("xG Predict gap closure", containing
S-199–S-205) are both titled `## Epic 13` — confirmed a genuine 2026-08-31
numbering slip, not a deliberate reuse, via `docs/CHANGELOG.md`'s own entry
for that date ("`docs/backlog.md` (**new Epic 13**, S-199 through S-205)").
At least 15 references across `NOTES.md`, `docs/requirements-document.md`,
`docs/CHANGELOG.md`, and `docs/decisions/0101-*.md` cite "Epic 13"
unqualified — only recoverable from surrounding context (S-number/date),
not the text itself. Unlike #11–#15 above, this is a documentation
consistency defect, not a code-duplication finding — flagged here anyway
since it's the same "a numbering/identity invariant silently broke and
nothing caught it" shape this report's own "no facade reintroduced"/"no
epic number reused" checks exist to catch. **Fix:** renumber the
later-written occurrence and correct every dependent cross-reference
individually (checked against its own surrounding context, not
batch-replaced) — `docs/backlog.md` Epic 30 S-236, flagged for `doc-sync`
rather than executed solo in this pass, since misreading even one
reference while renumbering would actively corrupt it. **Confirmed
landed by `doc-sync` and re-verified this revision:** exactly one
`## Epic 13` header remains (`grep -oE "^## Epic [0-9]+" docs/backlog.md
| sort | uniq -c` shows no count above 1); the later, non-canonical
occurrence (S-199–S-205, xG Predict gap closure) is now `## Epic 31`,
left in its original file position per `doc-sync`'s own reasoning (unique
numbers, not file-position-matching-order, is what the criteria required).
...
- **`backend/src/XGArcade.Api/CompositionRoot/ServiceRegistration.cs`** (761 lines, 18 commits — now the single highest-churn *source* file in the repo, up from 308 lines/5 commits) — re-read in full this pass given the growth. Still one line/block per registered service/option with its own doc comment, zero duplicated registration shape; the growth is entirely five new games' worth of registrations added in the file's own established pattern. Same verdict as before, re-confirmed under substantially more growth. Not a finding.
- **`backend/src/XGArcade.DataSync/Wikidata/WikidataClient.cs`** (1,028 lines, up from 782 at S-155) — every method added since (trophy-stats/international-stats/sitelink-count queries for xG Higher/Lower) read individually; each is a thin wrapper over the existing shared `RunIntersectionQueryAsync`/`RunThrowingQueryAsync` drivers. Not a finding — breadth without duplication, same verdict as `ServiceRegistration.cs` above.
- **`frontend/src/social/*.tsx`** (`FriendsScreen.tsx`/`FriendsTab.tsx`/`ChallengesTab.tsx`/`MatchmakingTab.tsx`, new for REQ-1401-1403): already share a `FetchListSection.tsx` (43 lines) component for their common fetch-a-list-and-render shape, applied by the delivery session itself rather than needing a sweep to catch it later — `docs/coding-guidelines.md`'s code-health-budget section (ADR-0084) working as intended. Not a finding.
- **`backend/src/XGArcade.Games.XGPredict/XGPredictGameModule.cs`** (294 lines, new) — the one `IGameModule` implementation without a Grid/Path-style generation-service split. Read in full: nine cohesive methods, largest one (`GenerateInstanceAsync`, ~90 lines) a single match-selection algorithm, not multiple concerns — well below the ~600-line size that triggered the earlier splits. Watch-only, not a finding; revisit past ~450 lines or a second concern.
- **ADR-0003 boundary re-verified across all five games** (not just the two that existed at the last check): grepped `backend/src/XGArcade.Core/` and `backend/src/XGArcade.Data/Entities/` for any `XGArcade.Games.*` reference — the only hits are doc comments citing a `GameKey` constant by name, never a real type/namespace reference. Clean.
- REQ-1420 (`ConnectChainStepDisputeService.cs` 297 lines, `ConnectChainStepDisputeEndpoints.cs` 188 lines) and S-234/ADR-0114 (`RoundGenerationService.cs`, 210 lines, unchanged in shape from before the renewal branch) — both read in full. Cohesive, appropriately sized, no duplication. Not findings.
...
Re-verified fresh this pass (fresh `npm install`, since `node_modules/` isn't checked in): `npm audit --omit=dev` **0 vulnerabilities** (production dependencies clean, re-confirmed). Full `npm audit` now surfaces **3** dev-only advisories (up from 1 at the last revision): `nanoid@<3.3.18` (transitive, via `vite`) — GHSA-2v37-7h3g-55p8, unchanged since 2026-08-18 — plus a newly-surfaced `@vitest/mocker` path-traversal advisory (GHSA-82fw-gwwq-j7x9, moderate, transitive via `vitest` itself). Both are dev-tooling-only (never bundled into the shipped frontend), no direct application call site, both Dependabot's routine-drift lane per `CLAUDE.md`, not fixed directly here. No hardcoded secrets (re-grepped across the new xG Connect/Higher-Lower/avatar-upload code specifically); no `FromSqlRaw`/`ExecuteSqlRaw`; `WikidataQid.IsValid` guard still centralized; CORS still an explicit allow-list; no `eval()`/`innerHTML`/`dangerouslySetInnerHTML` in production code, including the new `AvatarModerationSection.tsx`/`ChainBuilder.tsx` (both render only text/known-safe attributes). NuGet package versions not re-verified this pass (no `dotnet` SDK in this sandbox, confirmed again — `dotnet: command not found`). **No P1 security findings.**
