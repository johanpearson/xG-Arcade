...
// management, Escape-to-close, dialog markup) is shared via
// ScoringExplainerShell (../components/) — originally duplicated here, but
// once a third game (xG Predict, PredictScoringExplainer) needed the same
// shell, this comment's own prior "if a third game ever needs the same
// shell, that's the point to extract it, not before" was acted on; see that
// component's own doc comment for the extraction.
//
...
export function PathScoringExplainer({ onClose }: PathScoringExplainerProps) {
  return (
    <ScoringExplainerShell
      onClose={onClose}
      backdropClassName="path-scoring-explainer-backdrop"
      dialogClassName="path-scoring-explainer"
      headerClassName="path-scoring-explainer__header"
      closeClassName="path-scoring-explainer__close"
    >
