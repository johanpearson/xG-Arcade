...
//
// The modal shell itself (focus management, Escape-to-close, dialog markup)
// is shared via ScoringExplainerShell (../components/) — extracted once a
// third per-game explainer (PredictScoringExplainer) made it a third
// near-identical copy; see that component's own doc comment. Only the shell
// is shared — each game's actual content below stays a separate component.
export function ScoringExplainer({ onClose }: ScoringExplainerProps) {
  return (
    <ScoringExplainerShell
      onClose={onClose}
      backdropClassName="scoring-explainer-backdrop"
      dialogClassName="scoring-explainer"
      headerClassName="scoring-explainer__header"
      closeClassName="scoring-explainer__close"
    >
